...
const useRingtone = (active: boolean) => {
  ...
  useEffect(() => {
    ...
    const playRing = (startAt: number) => {
      if (stopped) return;
      // Two-tone ring: 480Hz + 440Hz for 1 second, silence for 2 seconds
    ...
    // Repeat ring every 3 seconds
    const ringInterval = 3.0;
    let i = 0;
    const schedule = () => {
      if (stopped) return;
      playRing(ctx.currentTime + i * ringInterval);
      i++;
      // Schedule next ring
      setTimeout(schedule, ringInterval * 1000);
    };
    schedule();
...
const IncomingCallModal = ({ callerName, callType, onAccept, onReject }: IncomingCallModalProps) => {
  useRingtone(true);
